import pickle
import streamlit as st
import requests

st.header('Movies Recommender System')
movies = pickle.load(open('movies_list.pkl','rb'))
similarity = pickle.load(open('Similarity.pkl','rb'))
movies_list = movies['title'].values
Selected_movie = st.selectbox(
    "Type or select a movie",
    movies_list
)

def fetch_poster(movie_id):
    url = "https://api.themoviedb.org/3/movie/{}?api_key=dcdc16fb5f850730a00c6b5201d51365&language=en-US".format(movie_id)
    data = requests.get(url)
    data = data.json()
    poster_path =data['poster_path']
    full_path = "https://image.tmdb.org/t/p/w500/"+poster_path
    return  full_path

def recommend(movie):
    index_movie = movies[movies['title']==movie].index[0]
    distances = sorted(list(enumerate(similarity[index_movie])),reverse = True, key = lambda x:x[1])
    recommended_movie_names = []
    recommended_movies_poster = []
    for i in distances[1:6]:
        movie_id = movies.iloc[i[0]].movie_id
        recommended_movie_names.append(movies['title'].iloc[i[0]])
        recommended_movies_poster.append(fetch_poster(movie_id))
        # st.image(recommended_movies_poster[0])
    return  recommended_movie_names,recommended_movies_poster

if st.button('Show Recommendation'):
    recommended_movie_names,recommended_movies_posters = recommend(Selected_movie)
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        st.text(recommended_movie_names[0])
        st.image(recommended_movies_posters[0])
    with col2:
        st.text(recommended_movie_names[1])
        st.image(recommended_movies_posters[1])

    with col3:
        st.text(recommended_movie_names[2])
        st.image(recommended_movies_posters[2])
    with col4:
        st.text(recommended_movie_names[3])
        st.image(recommended_movies_posters[3])
    with col5:
        st.text(recommended_movie_names[4])
        st.image(recommended_movies_posters[4])
